...
5. Run local recognition:
   ...
   ```bash
   python -m src.recognize --target-name name
   ```
...
- Movement topic: `vision/name/ne/movement`
- Status topic: `vision/name/ne/status`

Movement payloads on `vision/name/ne/movement`:
...
Dashboard JSON is published on `vision/name/ne/status`, including movement, lock state, target name, face count, confidence, distance, raw and smoothed horizontal error, center-zone/deadzone values, FPS, timing, threshold, provider, resolution, and MQTT health.

Useful addon flags:

```bash
python addons/mqtt_servo_tracking/recognize_mqtt.py --target-name name --mqtt-broker broker.hivemq.com --mqtt-topic vision/name/ne/movement --mqtt-status-topic vision/name/ne/status --camera-width 960 --camera-height 540 --max-faces 5 --locked-max-faces 5 --detect-every 2 --recognize-every 3 --landmark-roi-width 224 --deadzone-px 70 --center-zone-ratio 0.36 --center-exit-hysteresis-px 45 --command-hold-sec 0.25 --scan-delay-sec 0.8 --reacquire-hold-sec 0.30 --command-confirm-frames 2 --mqtt-min-interval 0.15 --mqtt-status-min-interval 0.25
```
...
3. Confirm:
   ...
   MQTT_TOPIC = "vision/name/ne/movement";
   ```
...
- Run `recognize_mqtt.py --target-name name` and lock the authorized speaker.
